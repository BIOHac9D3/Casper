"""
Casper Node - Web Backend API

FastAPI-based REST API for Casper Node
"""
...
async def health_check() -> Dict[str, Any]:
    ...
    return {
        "status": "ok",
        "message": "Casper Node API is running",
        "version": "0.1.0"
    }
...
async def get_environment() -> Dict[str, Any]:
    ...
    return {
        ...
        "home_directory": str(env.home_directory),
        "temp_directory": str(env.temp_directory),
        "current_directory": str(env.current_directory)
    }
...
async def execute_skill(skill_id: str, request: GenerateRequest) -> Dict[str, Any]:
    """Execute a specific skill."""
    # Force skill execution by adding skill-specific parameter
    result = skill_agent.generate(
        prompt=request.prompt,
        model=request.model,
        use_skills=True,
...
async def import_skills(repo_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "status": "ok",
            "message": f"Imported {count} skills",
            "count": count
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
